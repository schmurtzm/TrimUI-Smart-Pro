...
search_on_screenscraper() {
    ...
    if [ -z "$Head_api_result" ]; then
        ...
        echo "$(date '+%Y-%m-%d %Hh%M') : Request failed for ${romNameTrimmed}" >> /mnt/SDCARD/System/usr/trimui/scripts/scraper/scrap.log
        let Scrap_Fail++
        return
    fi
    ...
    if echo "$Head_api_result" | grep -q "^Erreur"; then
        ...
        echo "$(date '+%Y-%m-%d %Hh%M') : Couldn't find a match for ${romNameTrimmed}" >> /mnt/SDCARD/System/usr/trimui/scripts/scraper/scrap.log
        return
    fi
